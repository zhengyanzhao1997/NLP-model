...
def preceed_data(data,Max_len,sentence_max,tokenizer,train=False):
    ...
    sentence_label = np.zeros((data_len,sentence_max), dtype='int32')
    ...
    if  train:
        return {'ids':ids,'att':mask_att,'token_type':token_type,'sentence_index':sentence_index,'sentence_mask':sentence_mask,'sentence_label':sentence_label}
    return ({"ids":ids,'att':mask_att,'token_type':token_type,'sentence_index':sentence_index,'sentence_mask':sentence_mask},sentence_label)
...
def build_model(pretrained_path, bert_cofig, token_max, sentence_max):
    ...
    model_train = tf.keras.models.Model(inputs=[ids, att, token_type, sentence_index, sentence_mask, sentence_label],
                                        outputs=out_put_2)
    model_evaluate = tf.keras.models.Model(inputs=[ids, att, token_type, sentence_index, sentence_mask],
                                           outputs=out_put_1)
...
class Evaluator(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if  eva >= self.best_metric:  # 保存最优
            ...
            self.model_evaluate.save_weights('./model_save_2/extract_model_%s.hdf5' % self.fold)
...
def main():
    ...
    for fold in range(num_folds):
        ...
        train_data = preceed_data(data, token_max, sentence_max, tokenizer, train=True)
