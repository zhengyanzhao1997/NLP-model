...
def evaluate(data,pred,threshold=0.2):
    ...
    for d,yp in tqdm(zip(data,pred),desc='evaluating'):
        if len(d[0])>400:
            d[0] = d[0][:400]
        ex_result = [[d[0][i],yp[i]] for i in range(len(d[0]))]
        #yp = np.where(yp > threshold)[0]
        #pred_sum = ' '.join([d[0][i] for i in yp])
        summary = ' '.join(d[1])
        data_result.append({'extract':ex_result,'summary':summary})
    return data_result
...
def main():
    ...
    data = load_data('train_to_conver.json')
    
    for sp in range(4):
        data_x = np.load('train_conver.npy')
        data_x = data_x[:,:400]
        ...
        for f in range(fold):
            ...
            model.load_weights('./model_add/extract_model_%s.hdf5' % f)
            pred = model.predict(data_x)[:,:,0]
            pred_result += pred
        pred_result = pred_result/fold
        data_result = evaluate(data,pred_result)

        with open('./train_pred_result.json', 'w', encoding='utf-8') as f:
