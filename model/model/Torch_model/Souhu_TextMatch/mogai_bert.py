...
"""PyTorch BERT model. """
from tools import ConditionalLayerNorm
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.conditional_size:
            self.LayerNorm = ConditionalLayerNorm(config.hidden_size, eps=config.layer_norm_eps,
                                                  conditional_size=config.conditional_size)
        else:
    ...
    def forward(
        ...
        if self.conditional_size:
            embeddings = self.LayerNorm(embeddings, conditional)
        else:
...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, conditional=None):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        if self.conditional_size:
            hidden_states = self.LayerNorm(hidden_states + input_tensor, conditional)
        else:
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.output(self_outputs[0], hidden_states, conditional)
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        if self.conditional_size:
            layer_output = self.feed_forward_chunk_conditional(attention_output,conditional)
        else:
    ...
    def feed_forward_chunk_conditional(self, attention_output, conditional):
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output,conditional)
        return layer_output
