...
class Evaluator(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        #if score > self.score:
            self.model.save_weights('./pagesus_pretrain/best_model.hdf5')
            #self.score = score
    # 演示效果
...
def main():
    ...
    decode_max_len = 600
    batch_size = 2
    data  = load_data('/home_zyz/abstract_generate/final_abdata/union_add_noabs_cleaned_1920.json')
    random.shuffle(data)
    print(len(data))
    print(data[0][0])
    print(data[0][1])
    ...
    epochs = 50
    autotitle = AutoTitle(start_id=tokenizer.pad_token_id, end_id=tokenizer.eos_token_id,maxlen=599,max_decode_len = decode_max_len,model=model)
